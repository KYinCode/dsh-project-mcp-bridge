...
**连接 supervisor（v3）**：服务器的进程若死亡，SDK 的 `onclose` 触发，
死池条目被丢弃，该项目的所有运行中会话自动重建（约数秒）——无需重启、
无需新会话、无需改配置。重建失败不会自行重试（无重连循环）；下一次
触发（配置变化、新会话、再次死亡）会再试。注意：重建后，服务器自身的
**内部依赖**（如浏览器连接）可能还需数秒就绪——该窗口内的调用可能以
